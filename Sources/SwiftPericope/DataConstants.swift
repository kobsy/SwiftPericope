...
// MARK: - Chapters per Book

extension Pericope {
    static let chaptersPerBook: [Book:Int] = [
        .genesis: 50,
        .exodus: 40,
        .leviticus: 27,
        .numbers: 36,
...
// MARK: - Verses per Chapter

extension Pericope {
    static let versesPerChapter: [Int:Int] = [
        1001000: 31,
        1002000: 25,
        1003000: 24,
        1004000: 26,
...
extension Pericope {
    ...
    static var referencePattern: String {
        let number = "\\d{1,3}"
        let verse = "\(number)[\(validLetters.joined())]?"
        let chapterVerseSeparator = "\\s*[:\"\\.]\\s*"
        let listOrRangeSeparator = "\\s*[\\-–—,;]\\s*"
        let chapterAndVerse = "(?:\(number + chapterVerseSeparator))?\(verse)\\b"
        let chapterAndVerseOrLetter = "(?:\(chapterAndVerse)|[\(validLetters.joined())]\\b)"
        return "\(chapterAndVerse)(?:\(listOrRangeSeparator)\(chapterAndVerseOrLetter))*"
    }
    
    static var fragmentRegex = { try! NSRegularExpression(pattern: "^(?:(?<chapter>\\d{1,3}):)?(?<verse>\\d{1,3})?(?<letter>[\(validLetters.joined())])?$") }()
    
    static var validLetters = ["a", "b", "c", "d"]
}
